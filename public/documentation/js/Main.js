...
const createSelectLink = (item) =>
  addEvents(
    N(
      'a',
      item.level === 0 ? 'Home' : item.name.replace(Patterns.DISPLAY, ' '),
      {
        class: `${normalize(item.path)}${
          item.path === state.selection ? ' selected' : ''
        }`,
        ...{ href: `.?path=${item.path}` },
      }
    ),
...
class SearchOutput extends Viewable {
  ...
  searchChanged(results) {
    ...
    if (results && results.length > 0) {
      this.clear().append(
        results.map((result) => N('li', this.renderResult(result)))
      )
...
class Search extends Viewable {
  ...
  constructor() {
    super()
    this.view = N('div', [new SearchInput(), new SearchOutput(this)], {
      class: 'search',
    })
    state.setSearch('')
  }
    ...
    this.view = addEvents(
      N(
        ...
        N(
          'div',
          [
            N('div', chapter.name.replace(Patterns.DISPLAY, ' '), {
              class: 'chapter-card-title',
            }),
          ],
          {
            href: chapter.path,
...
class Chapters extends Viewable {
  constructor() {
    ...
    this.items = N('div')
    this.title = N('h3', 'Arena-X Help Desk')
    this.view = N(
      'section',
      [
        N(
  ...
  dataChanged(data) {
    ...
    return this.items.appendChild(
      N(
        'ul',
        data.children.map((chapter) => N('li', new ChapterCard(chapter)))
      )
    )
  }
}
...
class ReleaseSelection extends Viewable {
  ...
  createItem(item) {
    return N('option', item.name, {
      value: item.name,
      ...(item.name === state.releaseSelection ? { selected: 'selected' } : {}),
    })
  }
  ...
  releasesChanged(releases) {
    return this.clear().append(
      addEvents(
        N(
          'select',
          releases.map((item) => this.createItem(item))
        ),
        {
          change: (e) => {
            state.setReleaseSelection(e.target.value)
          },
        }
      )
    )
  ...
  selectionChanged(selection, content) {
    ...
    return this.clear()
      .append(
        N(
          'ul',
          list.map((item) => this.createItem(item))
        )
      )
...
class Navigation extends Viewable {
  constructor() {
    ...
    this.scroller = N('div', [this.menu, this.scrollTop, this.scrollBottom], {
      class: 'scroller',
    })
    this.view = N(
      'nav',
      [
        N('div', createSelectLink({ name: '', path: NavTools.getRoot() }), {
          class: 'home',
        }),
        this.scroller,
        this.toggle,
    ...
    const onIntersection = (entries) =>
      entries.forEach((entry) => {
        if (entry.target === this.menu.firstChild) {
          ...
          else this.scrollTop.classList.remove('hidden')
        } else if (entry.target === this.menu.lastChild) {
          if (entry.intersectionRatio > 0.9)
            this.scrollBottom.classList.add('hidden')
          else this.scrollBottom.classList.remove('hidden')
        }
      })
  ...
  createToggleButton() {
    return addEvents(N('button', '', { class: 'toggle' }), {
      click: () => {
  ...
  createScrollTopButton() {
    return addEvents(N('button', '↑', { class: 'scroll' }), {
      click: (() => {
  ...
  selectionChanged(selection, content) {
    clear(this.menu)
    ;[this.createItem(content.toplevel)]
      .concat(
...
class Content extends Viewable {
  constructor() {
    ...
    this.view = N(
      'article',
      [N('div', [this.breadcrumb, this.loader]), this.page],
      {
        class: 'content small',
      }
    )
  ...
  addHeadlineLink(item) {
    ...
    item.onclick = () => {
      history.replaceState(
        ...
        location.href.split('#')[0] + (item.id ? '#' + item.id : '')
      )
      item.scrollIntoView()
    }
  }
  ...
  replaceLinks() {
    ...
    ;[...root.querySelectorAll('h1, h2, h3, h4, h5, h6')].forEach(
      this.addHeadlineLink.bind(this)
    )
    root.styleSheets[0].insertRule(
      'h1:hover, h2:hover, h3:hover, h4:hover, h5:hover { text-decoration: underline; cursor: pointer; }'
    )
    root.styleSheets[0].insertRule('.headline { position: relative; }')
    root.styleSheets[0].insertRule(
      'a.github { position: absolute; width: 20px; height: 20px; bottom: 15%; right: 0px; background-color: #888888; display: none; }'
    )
  ...
  mdFromURL() {
    this.replacePage(N('zero-md', null))
  ...
  renderOverview(content) {
    return this.replacePage(
      N(
        'ul',
        content.children.map((chapter) => N('li', new ChapterCard(chapter))),
        { class: 'subchapter' }
      )
    )
  }
...
class Chapter extends Viewable {
  constructor() {
    ...
    this.view = N('section', [this.navigation, this.content], {
      class: 'chapter',
    })
  }
...
class App extends Viewable {
  ...
  constructor() {
    super()
    this.view = addEvents(document.body, {
      click: () => {
        state.setSearch([])
      },
    })
  ...
  loadReleases() {
    fetch('data/Releases.json')
      ...
      .then((releases) => state.setReleases(Array.from(new Set(releases))))
      .catch(() => {})
  }
  ...
  releaseSelectionChanged(releaseSelection) {
    fetch(
      `data/${
        releaseSelection.split('/').slice(-1)[0]
      }/${NavTools.getRoot()}.json`
    )
      .then((response) => response.json())
...
class ScrollHelper extends Viewable {
  constructor(header, footer) {
    ...
    this.scrollTop = addEvents(N('button', '⏶'), {
      click: () => window.scrollTo({ top: 0, behavior: 'smooth' }),
    })
    this.scrollBottom = addEvents(N('button', '⏷'), {
      click: () =>
        window.scrollTo({
    ...
    this.view = N('div', [this.scrollTop, this.scrollBottom], {
      class: 'scroll-helper',
    })
    this.watchScroll()
  }
      ...
      N(
        ...
        N(
          ...
          [
            N(
              ...
              [
                ...
                addEvents(
                  N('img', null, {
                    alt: content.name,
                    src: `/assets/images/icons/newtab.png`,
                  }),
                  {
            ...
            N('div', [N('p', 'Commit ID: ' + content.commitId)], {
              class: 'aboutBody',
            }),
          ],
...
class LegalNotice extends Viewable {
  constructor() {
    super()
    this.view = addEvents(N('div', 'About', { class: 'copy about' }), {
      click: () => this.overlay.appendTo(document.body),
    })
    this.loadData()
  }
...
class Footer extends Viewable {
  constructor() {
    ...
    this.view = N('footer', [
      N('div', '', { class: 'links' }),
      N('div', 'Copyright © Arena-X Network.', { class: 'copy' }),
      new LegalNotice(),
    ])
  }
}
